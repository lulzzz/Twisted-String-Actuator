 ...
 */
 ...
 #define enc1FastInPos 21
 #define enc1FastInNeg 20
 #define enc2FastInPos 19
 #define enc2FastInNeg 18
 ...
 #define MAX_PWM1 100 // max PWM to limit speed of motor (or use lower voltage) since encoder can't keep up with counts per second
 #define MAX_PWM2 110 //
 ...
 #define incThresh 500  // encoder counts per update from encoder board (make sure this number is the same on the uno)
...
void setup() {
  ...
  pinMode(enc1FastInPos, INPUT_PULLUP);
  pinMode(enc1FastInNeg, INPUT_PULLUP);
  pinMode(enc2FastInPos, INPUT_PULLUP);
  pinMode(enc2FastInNeg, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(enc1FastInPos), e1p, RISING);
  attachInterrupt(digitalPinToInterrupt(enc1FastInNeg), e1n, RISING);
  attachInterrupt(digitalPinToInterrupt(enc2FastInPos), e2p, RISING);
  attachInterrupt(digitalPinToInterrupt(enc2FastInNeg), e2n, RISING);

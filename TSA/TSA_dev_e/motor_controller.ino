...
//     motor2pwm = 0; }
 ...
 void motorDrive (int motor_channel, int motor_pwm) {
  ...
  // set different MAX PWMs because motors are behaving differently (same thing a making separate gains for each motor)
  int MAX_PWM = 255;
  if (motor_channel == 1) {
    MAX_PWM = MAX_PWM1;}
  ...
  int pwm_out = min(abs((int)motor_pwm),MAX_PWM);
